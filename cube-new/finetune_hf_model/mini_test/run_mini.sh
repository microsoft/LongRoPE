...
if [ -z "$EXAMPLE_PATH" ]; then
    EXAMPLE_PATH=$HOME/Fairseq/cube_examples/finetune_hf_model
fi
...
if [ 0 -ne $USE_LORA ]; then
    MODEL_PARAMS="$MODEL_PARAMS $LORA_CONFIG"
fi
...
DATASET_PATH=$SCRIPT_PATH/alpaca_data_100.json
PROMPT_TYPE=alpaca
...
--tokens-per-sample $SEQUENCE_LENGTH \
--pad-to-fixed-length \
--disable-validation \
...
then
    ...
    $CUBE_ENV torchrun --nproc_per_node=1 --nnodes=1 --master_addr $MASTER_ADDR --master_port $MASTER_PORT $EXAMPLE_PATH/train.py $TASK_PARAMS --num-workers 0 $CUBE_PARAMS --adam-betas "(0.9,0.999)" --compile=compile_only $AUTODIST_PARAMS --cube-load-graph-ckp $GRAPH_PATH >$LOG_DIR/compile_log.txt 2>&1
elif [ $MODE = "run" ]
then
