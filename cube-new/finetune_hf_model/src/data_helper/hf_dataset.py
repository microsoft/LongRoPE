...
class WrappedHFDataset(FairseqDataset):
    def __init__(self, data_name_or_path: str,
        ...
        if if_tokenize:
            ...
            # Redpajama
            if data_func == 'Redpajama_tokenize_yarn':
            ...
            if data_func == 'Redpajama_tokenize_split_long_short':
                print("Data - Redpajama")
                data_num_proc = 4
                # self.dataset = 
            ...
            if data_func == 'Redpajama_Same_Doc_Fliter_2xdoc':
                print("Data - Redpajama same doc")
                print(dataset)
                dataset = dataset['train']
                ...
                if "position_ids" not in train_dataset.column_names:
                    def add_position_ids(sample, tokenizer):
                        ...
                        chunked_ids = torch.cat((ids[0, lt1:rt1], ids[0, lt2:rt2]))
                        # input_ids.append(chunked_ids)
                        # print("chunked_ids.shape", chunked_ids.shape)
                        
                        # if chunked_ids.shape[0] < model_max_position_embeddings:
                        ...
                        sample["position_ids"] = pos_ids.clone().detach()
                        sample["attention_mask"] = torch.ones(
                        len(chunked_ids), dtype=torch.int8)
                        # print(sample)
                        # exit(0)
                        return sample
                    ...
                    train_dataset_2 = load_from_disk("/mnt/yiran/cache/RedPajama-4k-pose-128k-same-doc2-fliter")
                    
                    print("train_dataset_2", train_dataset_2)
                    ...
                    def filter_func(x):
                        return len(x['input_ids']) >= int(os.environ['SEQ_LEN'])
                    train_dataset_1 = train_dataset_1.filter(filter_func, desc="fliter short", num_proc=data_num_proc)
                    
                    
                    # Concatenate the two datasets
                    train_dataset = concatenate_datasets([train_dataset_1, train_dataset_2])
            ...
            if data_func == 'Redpajama_Same_Doc_Fliter_12xdoc':
                print("Data - Redpajama same doc")
                print(dataset)
                dataset = dataset['train']
                train_dataset = dataset
                ...
                LONGROPE_POSE_SEQ = int(os.environ['LONGROPE_POSE_SEQ'])
                ...
                def doc12x(idx, sample, tokenizer, data_dict):
                    flag = True
                    inputs = sample["text"]
                    raw_model_inputs = tokenizer(
                        inputs, 
                    ...
                    if len(ids) < SEQ_LEN:
                        ...
                        # generate idx
                        lt1 = 0
                        rt1 = random.randint(1, (len_chunk+1)//2) # 2k
                        rt2 = random.randint(lt1+len_chunk, len_input) # 6k
                        lt2 = rt2 - (len_chunk - (rt1-lt1)) # 4k
                ...
                data_dict = {'input_ids': [], 'attention_mask': [], 'position_ids': []}  
                for i in tqdm(range(train_dataset.num_rows)):
                    # if i%10000==0: print(i)
                    data_dict = doc12x(i, train_dataset[i], tokenizer, data_dict)
    ...
    def collater(self, samples):
        ...
        # If rerun failed:
        # pad_input_ids = mini_batch.pop('input_ids')
        # if len(pad_input_ids) < os.environ['SEQ_LEN']:
        #     pad_input_ids += [self.tokenizer.pad_token_id] * (os.environ['SEQ_LEN'] - len(pad_input_ids))
        # _mini_batch['src_tokens'] = pad_input_ids
