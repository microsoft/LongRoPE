...
class LlamaLongRoPEScaledRotaryEmbedding(LlamaRotaryEmbedding):
    ...
    #     # print("$register_buffer emb_cos", emb_cos.shape)
    ...
    def forward(self, x, seq_len=None):
        ...
        if 'LONGROPE_POSE_SEQ' in os.environ:
            if seq_len < int(os.environ['LONGROPE_POSE_SEQ']):
                print("$use LONGROPE_POSE_SEQ", int(os.environ['LONGROPE_POSE_SEQ']))
                seq_len = int(os.environ['LONGROPE_POSE_SEQ'])
        
        if seq_len > self.original_max_position_embeddings:
            t = torch.arange(seq_len, device=x.device, dtype=torch.float32)
            rescale_factors = self.rescale_factors.to(x.device)
        else:
            t = torch.arange(self.original_max_position_embeddings, device=x.device, dtype=torch.float32)
            rescale_factors = torch.ones(self.rescale_factors.shape, dtype=torch.float32, device=x.device)
