...
def rms_norm(x, weight: Optional[torch.Tensor], eps: float = 1e-6):
    ...
    from CubeInfer.model_executor.kernels.triton.rmsnorm_kernels import _rms_norm_forward
...
def rms_norm_add(
    ...
    if torch.is_grad_enabled() and (
        ...
        raise ValueError("Gradients not supported.")
    assert _is_triton_available()
    from CubeInfer.model_executor.kernels.triton.rmsnorm_kernels import _rms_norm_add_forward
