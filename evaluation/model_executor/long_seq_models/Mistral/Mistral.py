...
@cube.graph.parser.register('bs q_length (kv_num_heads 4) head_dim, \
    bs q_length kv_num_heads head_dim, \
        bs q_length kv_num_heads head_dim, \
            bs seq_length tp_num_heads^ head_dim, \
                bs seq_length tp_num_heads^ head_dim \
            -> bs q_length (kv_num_heads 4 head_dim)')
...
class MistralAttention(nn.Module):
    ...
    def forward(
        ...
        if past_key_value is not None:
            output = flash_attention_with_kvcache(q, k, v, past_key_value[0], past_key_value[1], target_length, bsz, q_len, sliding_window_attention)
        else:
