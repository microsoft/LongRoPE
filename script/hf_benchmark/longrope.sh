...
WINOGRANDE="--tasks=winogrande --num_fewshot=5"
GSM8K="--tasks=gsm8k --num_fewshot=5"
...
MODEL_ARGS="model=${BASE_PATH}${MODEL_PATH},method=${METHOD},factor=${FACTOR},s_pi_para=${SPI_PARA},original_max_position_embeddings=4096,cache_dir=./cache_dir"
...
python evaluation/lm-evaluation-harness/main.py \
    ...
    ${ARGS} \
    ${WINOGRANDE} \
    --output_path="${OUTPUT_PATH}/${MODEL_PATH}-winogrande-${METHOD}-${MARK}.json"
...
python evaluation/lm-evaluation-harness/main.py \
    ...
    ${ARGS} \
    ${GSM8K} \
    --output_path="${OUTPUT_PATH}/${MODEL_PATH}-gsm8k-${METHOD}-${MARK}.json"
