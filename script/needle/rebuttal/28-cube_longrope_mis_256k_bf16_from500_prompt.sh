...
nums=28
name="${nums}-cube_longrope_mistral_256k_needle_origin"
...
for prompt_name in "${prompt_list[@]}"; do  
    name="${nums}-cube_longrope_mistral_256k_prompt_${prompt_name}_needle_origin"
    ...
    python evaluation/needle/visualize.py --name ${nums}-${prompt_name}-origin --path evaluation/needle/result/$name/ck-${ck_step}/
done  
