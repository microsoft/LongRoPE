...
for file_idx in $(seq 0 5 100 ); do  
    name="29-cube_longrope_mis_256k_bf16_from500_ck_${ck_step}_debug_needle_origin_file_${file_idx}"
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 $torch_path/torchrun \
        ...
        --seq_series "400000,500000,700000,900000" \
        --city_idx 15 \
        --random_num 6072345 \
        --file_order_idx $file_idx \
        --document_depth_percent_intervals 5 \
        --model_provider Mistral \
        --model_path ${mistral_256k} \
        --result_path ./evaluation/needle/result/$name/ \
    ...
    python evaluation/needle/visualize.py --name 29-file_idx-bf16-256k-origin-file-${file_idx} --path evaluation/needle/result/$name/ck-$ck_step/
