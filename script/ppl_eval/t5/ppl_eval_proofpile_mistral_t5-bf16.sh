...
setting["longrope_mistral_128k_bf16"]="--model ${path_team}/ft_out_model/cube-mis-128k-bf16/ck-1_1000 --method longrope  --finetuned --factor 32 --original_max_position_embeddings 4096"

setting["longrope_mistral_256k_bf16_from_step500"]="--model ${path_team}/ft_out_model/cube-mis-256k-bf16-step-500/ck-1_500 --method longrope  --finetuned --factor 64 --original_max_position_embeddings 4096"
...
config_list=("longrope_mistral_256k_bf16_from_step500")
...
for config in "${config_list[@]}"; do
    for max_tokens in "${max_tokens_list[@]}"; do
        ...
        python evaluation/perplexity.py \
            ...
            --tokens_step 2048 \
            --output_file "${output_dir}/t5_proofpile_${config}_${max_tokens}.csv" \
            --sliding_window_attention 262144 \
            --flash_attn \
            ${save_memory} \
            --cache_dir $cache_dir
    done
